...
string OpenOrder(PGconn *conn, int &order_id, int account_id, int symbol_id,
    ...
    std::string query =
        "INSERT INTO orders (account_id, symbol_id, order_type, "
...
void create_tables(PGconn *conn) {
    const char *create_symbols_table =
...
PGconn *init_db() {
    const char *conninfo =
        "host=stock_db port=5432 dbname=postgres "
...
std::string ExecuteOrder(PGconn *conn, std::pair<int, double> matched_order,
    ...
    std::string query =
        "SELECT limit_price, account_id, symbol_id "
        "FROM orders "
        "WHERE order_id = " +
    ...
    query =
        "INSERT INTO executed_orders (order_id, amount, execution_price, "
        "timestamp) "
        "VALUES (" +
        std::to_string(order_id) + ", " +
